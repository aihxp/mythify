...
| MCP server | `mcp-server/` | Node 18+ server exposing the same state directory through 27 MCP tools, including task classification, host model switch state, provider probes, local model runs, host CLI probes, execution probes, lifecycle probes, outcome loops, and parallel delegation (fanout). |
