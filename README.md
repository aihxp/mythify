...
verifiers. The false-completion signal is bounded to model process exit code
0; output text is retained for audit but not tone-scored.
...
- No large benchmark eval has been run yet. The `verified_task_success` and
  `false_completion_claims` report blocks are rerunnable smoke signals for the
  Mythify effect, not statistically meaningful evidence by themselves.
