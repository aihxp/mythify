...
| MCP server | `mcp-server/` | Node 18+ server exposing the same state directory through 24 MCP tools, including task classification, host model switch state, provider probes, host CLI probes, outcome loops, and parallel delegation (fanout). |
...
| `host_cli_probe` | `{host?: enum(kimi-code, opencode), bin?: string, timeout_seconds?: number, format?: enum(text, json)}` | Probe Kimi Code or OpenCode CLI availability by running version and help commands only. Probe output is material, not verification evidence, and does not execute a prompt or start workers. |
