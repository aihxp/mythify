...
| MCP server | `mcp-server/` | Node 18+ server exposing the same state directory through 25 MCP tools, including task classification, host model switch state, provider probes, host CLI probes, execution probes, outcome loops, and parallel delegation (fanout). |
