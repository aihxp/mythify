...
the fix. Remediation status has since been updated in place through v3.6.51.
...
| Testing and Verification | 84 | B | 15% | 288 tests, no theater, deterministic, gate edges and extracted helper modules covered. |
...
- [ ] [QUAL-001] Two ~10k-line god-modules - Open. Progress in v3.6.28: deterministic classification was extracted to direct-import Python and MCP modules with module-level tests. Progress in v3.6.29: host model switch record construction, capability enrichment, and formatting were extracted to direct-import Python and MCP modules with module-level tests. Progress in v3.6.30: trace analysis and trace playbook Markdown formatting was extracted to a direct-import Python module with direct module tests. Progress in v3.6.31: MCP provider probing and local model role-runner helpers were extracted to `mcp-server/src/model-provider.js` with direct module tests. Progress in v3.6.32: MCP host CLI probe and worker helpers were extracted to `mcp-server/src/host-cli.js` with direct module tests. Progress in v3.6.33: MCP execution adapter probe and run helpers were extracted to `mcp-server/src/execution-adapter.js` with direct module tests. Progress in v3.6.34: MCP lifecycle probe helpers were extracted to `mcp-server/src/lifecycle-adapter.js` with direct module tests. Progress in v3.6.35: MCP provider defaults, adapter contracts, and role assignment metadata were extracted to `mcp-server/src/provider-defaults.js` with direct module tests. Progress in v3.6.36: MCP model policy construction and model triage runner helpers were extracted to `mcp-server/src/model-policy.js` with direct module tests. Progress in v3.6.37: Python model policy construction, provider-default metadata, and model triage runner helpers were extracted to `scripts/mythify_model_policy.py` with direct module tests. Progress in v3.6.38: Python durable IO helpers were extracted to `scripts/mythify_io.py` with direct module tests. Progress in v3.6.39: Python research and campaign workflow stores were extracted to `scripts/mythify_workflows.py` with direct module tests. Progress in v3.6.40: Python prompt packet and workflow route helpers were extracted to `scripts/mythify_router.py` with direct module tests. Progress in v3.6.41: Python outcome loop store and command handlers were extracted to `scripts/mythify_outcomes.py` with direct module tests. Progress in v3.6.42: Python read-only dashboard, history, report, background, progress, readiness, timeline, and phase view builders and command handlers were extracted to `scripts/mythify_views.py` with direct module tests. Progress in v3.6.43: Python trace command handlers were moved into `scripts/mythify_trace.py` with direct module tests. Progress in v3.6.44: Python memory and lesson stores and command handlers were extracted to `scripts/mythify_memory.py` with direct module tests. Progress in v3.6.45: Python host-model state helpers and command handlers were moved into `scripts/mythify_host_model.py` with direct module tests. Progress in v3.6.46: MCP adapter and host integration tool registrations were moved into `mcp-server/src/adapter-tools.js` with direct module tests. Progress in v3.6.47: MCP read-only view tool registrations were moved into `mcp-server/src/view-tools.js` with direct module tests. Progress in v3.6.48: MCP memory and lesson tool registrations were moved into `mcp-server/src/memory-tools.js` with direct module tests. Progress in v3.6.49: MCP outcome loop tool registrations were moved into `mcp-server/src/outcome-tools.js` with direct module tests. Progress in v3.6.50: MCP plan tool registrations were moved into `mcp-server/src/plan-tools.js` with direct module tests. Progress in v3.6.51: MCP verification and reflection tool registrations were moved into `mcp-server/src/verification-tools.js` with direct module tests.
...
- **Test suite quality, not just quantity.** 156 Python + 128 MCP tests, more than 2,300 assertions, zero no-assertion/`assertTrue(True)` theater, no mocking (real subprocesses, real loopback HTTP on `127.0.0.1:0`, real exit codes). Deterministic (1s-timeout-vs-5s-sleep style). Preserve this style for any new code.
