...
the fix. Remediation status has since been updated in place through v3.6.38.
...
- Size: `scripts/mythify.py` 7,225 lines plus `scripts/mythify_classification.py` 432 lines, `scripts/mythify_host_model.py` 386 lines, `scripts/mythify_io.py` 245 lines, `scripts/mythify_model_policy.py` 1,606 lines, and `scripts/mythify_trace.py` 883 lines; `mcp-server/src/index.js` 5,600 lines plus `mcp-server/src/classification.js` 415 lines, `mcp-server/src/host-model.js` 318 lines, `mcp-server/src/model-policy.js` 1,052 lines, `mcp-server/src/model-provider.js` 470 lines, `mcp-server/src/host-cli.js` 514 lines, `mcp-server/src/execution-adapter.js` 460 lines, `mcp-server/src/lifecycle-adapter.js` 333 lines, and `mcp-server/src/provider-defaults.js` 415 lines; `mcp-server/src/fanout.js` 2,494; plus `capability-registry.js` (883) and small registry shims. ~23.6k lines of core code.
...
| Testing and Verification | 84 | B | 15% | 273 tests, no theater, deterministic, gate edges and extracted helper modules covered. |
