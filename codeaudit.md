...
the fix. Remediation status has since been updated in place through v3.6.35.
...
- Size: `scripts/mythify.py` 8,969 lines plus `scripts/mythify_classification.py` 432 lines, `scripts/mythify_host_model.py` 386 lines, and `scripts/mythify_trace.py` 883 lines; `mcp-server/src/index.js` 6,626 lines plus `mcp-server/src/classification.js` 415 lines, `mcp-server/src/host-model.js` 318 lines, `mcp-server/src/model-provider.js` 470 lines, `mcp-server/src/host-cli.js` 514 lines, `mcp-server/src/execution-adapter.js` 460 lines, `mcp-server/src/lifecycle-adapter.js` 333 lines, and `mcp-server/src/provider-defaults.js` 415 lines; `mcp-server/src/fanout.js` 2,494; plus `capability-registry.js` (883) and small registry shims. ~23.6k lines of core code.
...
| Testing and Verification | 84 | B | 15% | 270 tests, no theater, deterministic, gate edges and extracted helper modules covered. |
...
- Location: `scripts/mythify.py` (8,969-line entrypoint after v3.6.35; `build_parser` remains one large function; 61 `cmd_*` handlers still live mostly in one file). `mcp-server/src/index.js` (6,626-line entrypoint after v3.6.35; 37 inline `registerTool` blocks remain).
- Evidence: v3.6.28 extracts deterministic classification to `scripts/mythify_classification.py` and `mcp-server/src/classification.js`, with direct module tests. v3.6.29 extracts host model switch record construction, capability enrichment, and formatting to `scripts/mythify_host_model.py` and `mcp-server/src/host-model.js`, with direct module tests. v3.6.30 extracts trace analysis and trace playbook Markdown formatting to `scripts/mythify_trace.py`, with direct module tests. v3.6.31 extracts MCP provider probing and local model role-runner helpers to `mcp-server/src/model-provider.js`, with direct module tests. v3.6.32 extracts MCP host CLI probe and worker helpers to `mcp-server/src/host-cli.js`, with direct module tests. v3.6.33 extracts MCP execution adapter probe and run helpers to `mcp-server/src/execution-adapter.js`, with direct module tests. v3.6.34 extracts MCP lifecycle probe helpers to `mcp-server/src/lifecycle-adapter.js`, with direct module tests. v3.6.35 extracts MCP provider defaults, adapter contracts, and role assignment metadata to `mcp-server/src/provider-defaults.js`, with direct module tests. The main runtime files remain too large, with very large dispatch/registration functions and most command/tool logic still in the entrypoints.
