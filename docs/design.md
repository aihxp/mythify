...
(current 1.x) and `zod` (4.x). package.json: name `mythify-mcp`, version `3.6.11`,
...
This is Mythify v3.6.11. Fanout was added in 2.1.0; 2.2.0 added local
...
makes the MCP server read its reported version from `package.json`.
The CLI reports 3.6.11 through `--version`; the MCP server reads `package.json`
