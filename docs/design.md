...
Exactly 24 tools: the 21 core tools below plus the 3 fanout tools defined in the
...
| `host_cli_probe` | `{host?: enum(kimi-code, opencode), bin?: string, timeout_seconds?: number, format?: enum(text, json)}` | Probe Kimi Code or OpenCode CLI availability by running only version and help commands. Defaults to `MYTHIFY_KIMI_BIN` or `MYTHIFY_OPENCODE_BIN`, then PATH and common install paths. Returns binary resolution, feature evidence, `can_run_noninteractive_prompt`, and `material_not_evidence: true`. It does not execute a prompt, write state, spawn workers, or count as verification evidence. |
...
1. `tools/list` returns exactly the 24 tool names above (set equality), the 21
...
8. A short MCP note listing the 24 tool names for clients using the server instead
