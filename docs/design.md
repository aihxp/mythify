...
(current 1.x) and `zod` (4.x). package.json: name `mythify-mcp`, version `3.6.9`,
...
This is Mythify v3.6.9. Fanout was added in 2.1.0; 2.2.0 added local
...
MCP server dependency audit gate to the Node CI matrix.
The CLI prints no version banner; the MCP server reports 3.6.9 through its
server info.
