...
- Load-bearing file sizes: `scripts/mythify.py` 6444 LOC,
  `mcp-server/src/index.js` 7404 LOC, `mcp-server/src/fanout.js` 2311 LOC,
  and `protocol/classification-rules.json` 194 LOC after the final backlog
  cleanup.
...
- Final backlog cleanup: deterministic classification keywords now live in
  `protocol/classification-rules.json`, reducing CLI/MCP drift without a broad
  handler refactor.
...
| [BACKLOG-001] High-churn classifier keyword table duplicated across CLI and MCP | Completed in this patch | `protocol/classification-rules.json` now owns deterministic classification keywords; both runtimes load it and tests check review terms through the manifest. |
